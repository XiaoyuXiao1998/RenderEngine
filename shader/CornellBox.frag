...
//function prototypes
vec3 CalcDirLight(vec3 light_dir,vec3 normal, vec3 view_dir);
//shadow map function
...
{    
    ...
    vec3 radiance = CalcDirLight(light_dir,norm,viewDir);
...
vec3 CalcDirLight(vec3 light_dir,vec3 normal, vec3 view_dir){
   ...
   // visibility = shadowCalculation(FragPosLightSpace);
    ...
    vec3 specular = LightIntensity * spec * color;

    return diffuse * (1-visibility);
}
